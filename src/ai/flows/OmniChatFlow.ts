 ...
 *
 * - invokeOmniChatFlow - Handles chat interactions, potentially using tools.
...
export async function invokeOmniChatFlow(
  input: OmniChatInput,
): Promise<OmniChatOutput> {
  const ai = getAi();
  const searchTool = getSearchTool();

  const omniChatGenkitFlow = ai.defineFlow(
    {
      name: 'omniChatFlow',
  ...
  return omniChatGenkitFlow(input);
}
